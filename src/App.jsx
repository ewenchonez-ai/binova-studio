...
// - les vidéos d'avant le 8 juin 2026 : on garde les données mais on retire la date (noDate=true)
// - les vidéos vides d'après le 8 juin 2026 sur l'ancien rythme (Lun/Mer/Ven) : on les vire
//   pour les remplacer par les nouveaux slots tous les 3 jours
// - les vidéos remplies d'après le 8 juin : on les conserve telles quelles
function migrateAndMergeVideos(loaded) {
  // 1) Marquer les anciennes comme noDate
  const migrated = loaded.map(v =>
    v.date < SCHEDULE_CUTOFF ? { ...v, noDate: true } : { ...v, noDate: v.noDate || false }
  );

  // 2) Séparer les noDate (toujours conservées) des datées
  const noDateOnes = migrated.filter(v => v.noDate);
  const datedOnes = migrated.filter(v => !v.noDate);

  // 3) Retirer les vidéos vides sur l'ancien rythme (post-cutoff mais sans données)
  //    pour pouvoir les remplacer proprement par le nouveau rythme
  const datedKept = datedOnes.filter(v => !isEmptyVideo(v));
  const datedKeptByDate = Object.fromEntries(datedKept.map(v => [v.date, v]));

  // 4) Générer le nouveau planning (tous les 3 jours)
  const fresh = generateSlots(START_DATE, DEFAULT_END);

  // 5) Fusionner : pour chaque date du nouveau planning, si on a déjà des données pour cette date, on les garde
  const merged = fresh.map(slot =>
    datedKeptByDate[slot.date] ? { ...slot, ...datedKeptByDate[slot.date], noDate: false } : slot
  );

  // 6) Ajouter aussi les vidéos remplies dont la date ne tombe pas sur le nouveau rythme
  const mergedDates = new Set(merged.map(v => v.date));
  const orphans = datedKept.filter(v => !mergedDates.has(v.date));

  // 7) Ordre final : noDate en haut, puis vidéos triées par date
  return [
    ...noDateOnes,
    ...[...merged, ...orphans].sort((a, b) => a.date.localeCompare(b.date)),
  ];
}
