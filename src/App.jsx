...
function dateForVideoNumber(num) {
  const FIRST_NUMBER_DATED = 31; // La vidéo 31 = première datée
...
// Migration des vidéos existantes vers le nouveau planning :
// - vidéos avec "Vidéo N" dans le titre où N < 31  → sans date (gardent leurs données)
// - vidéos avec "Vidéo N" dans le titre où N >= 31 → date = 8 juin + (N-31)*3 jours
// - vidéos avec données mais sans numéro identifiable → sans date
// - slots vides (= placeholders du planning précédent) → ignorés et remplacés par le nouveau rythme
function migrateAndMergeVideos(loaded) {
  ...
  for (const v of loaded) {
    ...
    if (num !== null) {
      // Vidéo numérotée → décide date selon le numéro
      if (num < 31) {
        noDateList.push({ ...v, noDate: true });
      } else {
        const newDate = dateForVideoNumber(num);
        datedOverrides[newDate] = {
          ...v,
          date: newDate,
          id: `vid_${newDate}`,
          noDate: false,
    ...
    } else if (!isEmptyVideo(v)) {
      // Pas de numéro mais a des données → sans date
      noDateList.push({ ...v, noDate: true });
    }
    // Sinon : vidéo vide sans numéro → on l'ignore (sera remplacée par le nouveau planning)
  }

  // Génère les slots vides du nouveau planning et remplace par les overrides quand on en a
  const fresh = generateSlots(START_DATE, DEFAULT_END);
  const merged = fresh.map(slot => datedOverrides[slot.date] || slot);

  // Ajoute les vidéos numérotées dont la date dépasse DEFAULT_END (au cas où)
  ...
  return [
    ...noDateList,
    ...[...merged, ...beyondEnd].sort((a, b) => a.date.localeCompare(b.date)),
  ];
}
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        if (v?.value) setVideos(migrateAndMergeVideos(JSON.parse(v.value)));
...
function PlanningView({ videos, updateVideo, thumbs, setVideoThumbnail }) {
  ...
  const months = useMemo(() => {
    const set = new Set(videos.filter(v => !v.noDate).map(v => v.date.slice(0,7)));
    return Array.from(set).sort();
  }, [videos]);

  const filtered = useMemo(() => videos.filter(v => {
    if (statusFilter !== 'all' && v.status !== statusFilter) return false;
    if (monthFilter === '__none__' && !v.noDate) return false;
    if (monthFilter !== 'all' && monthFilter !== '__none__' && (v.noDate || !v.date.startsWith(monthFilter))) return false;
    if (search) {
  ...
  const noDateCount = useMemo(() => videos.filter(v => v.noDate).length, [videos]);
  ...
  return (
    <div>
      ...
      <p style={{ fontSize:13, color:C.muted, margin:'8px 0 36px' }}>
        ...
        {noDateCount > 0 && <> · <span className="mono">{noDateCount}</span> sans date</>}
      </p>
      ...
      <div style={{ display:'flex', gap:8, marginBottom:20, alignItems:'center' }}>
        ...
        <select value={monthFilter} onChange={e => setMonthFilter(e.target.value)} style={selectStyle()}>
          ...
          {noDateCount > 0 && <option value="__none__">Sans date ({noDateCount})</option>}
          {months.map(m => {
            const [y, mo] = m.split('-');
    ...
    <div className="row" style={{ borderBottom:`1px solid ${C.line}` }}>
      <div style={{
        ...
        <div className="mono" style={{ fontSize:12 }}>
          {video.noDate
            ? <span style={{ color:C.muted2, fontStyle:'italic' }}>Sans date</span>
...
function CalendarView({ videos, updateVideo, thumbs, setVideoThumbnail }) {
  ...
  const byDate = useMemo(() => Object.fromEntries(videos.filter(v => !v.noDate).map(v => [v.date, v])), [videos]);
...
function Settings({ settings, setSettings, onClose, videos, setVideos }) {
  ...
  const regenerate = () => {
    ...
    const noDateOnes = videos.filter(v => v.noDate);
    const existing = Object.fromEntries(videos.filter(v => !v.noDate).map(v => [v.date, v]));
    const fresh = generateSlots(START_DATE, DEFAULT_END);
    const dated = fresh.map(slot => existing[slot.date] ? { ...slot, ...existing[slot.date], noDate:false } : slot);
    setVideos([...noDateOnes, ...dated]);
