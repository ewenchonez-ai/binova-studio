...
const RELEASE_DAYS = [1, 3, 5]; // legacy, conservé pour le calendrier (jours mis en évidence)
const INTERVAL_DAYS = 3; // Nouvelle cadence : une vidéo tous les 3 jours
const SCHEDULE_CUTOFF = '2026-06-08'; // Avant cette date : vidéos sans date assignée
const DAY_SHORT = ['Dim','Lun','Mar','Mer','Jeu','Ven','Sam'];
const DAY_FULL = ['Dimanche','Lundi','Mardi','Mercredi','Jeudi','Vendredi','Samedi'];
const MONTHS = ['Janvier','Février','Mars','Avril','Mai','Juin','Juillet','Août','Septembre','Octobre','Novembre','Décembre'];
const START_DATE = new Date(2026, 5, 8); // 8 juin 2026
...
function isEmptyVideo(v) {
  return !v.title && v.status === 'rien' && !v.rushLink && !v.finalLink
    && !v.notes && !v.views && !v.hasThumbnail && !v.telegramPosted;
}

// Fusion non-destructive : ajoute les slots vides du planning tout en préservant
// TOUTES les vidéos existantes qui ont des données (peu importe leur date).
function mergePlanningPreserving(existingVideos, startDate, endDate) {
  const nonEmpty = existingVideos.filter(v => !isEmptyVideo(v));
  const nonEmptyByDate = Object.fromEntries(nonEmpty.map(v => [v.date, v]));

  // Nouveau planning tous les 3 jours à partir de startDate
  const fresh = generateSlots(startDate, endDate);
  // Sur chaque slot du nouveau planning : garde les données existantes si on en a
  const merged = fresh.map(slot => nonEmptyByDate[slot.date] || slot);
  const mergedDates = new Set(merged.map(v => v.date));

  // Vidéos remplies dont la date ne fait pas partie du nouveau planning : préservées
  const orphans = nonEmpty.filter(v => !mergedDates.has(v.date));

  // Tri par date + dédoublonnage par ID (safety net)
  const combined = [...merged, ...orphans].sort((a, b) => a.date.localeCompare(b.date));
  const seen = new Set();
...
// Score de "complétude" pour départager les conflits (deux vidéos avec le même numéro de titre)
function videoCompleteness(v) {
  let s = 0;
  if (v.title) s += 1;
  if (v.status && v.status !== 'rien') s += 2;
  if (v.rushLink) s += 1;
  if (v.finalLink) s += 2;
  if (v.notes) s += 1;
  if (v.hasThumbnail) s += 2;
  if (v.views) s += 3;
  if (v.telegramPosted) s += 1;
  return s;
}

// Extrait le numéro de vidéo depuis le titre.
// Reconnaît : "Vidéo 31", "video 32", "Vidéo #33", "Vidéo33", "Video 34", etc.
function extractVideoNumber(title) {
  if (!title) return null;
  const m = title.match(/[Vv]id(?:é|e)o\s*#?\s*(\d+)/);
  return m ? parseInt(m[1], 10) : null;
}

// Calcule la date d'une vidéo à partir de son numéro (vidéo 31 = START_DATE, +3 jours par numéro)
function dateForVideoNumber(num) {
  const FIRST_NUMBER_DATED = 31;
  const offset = (num - FIRST_NUMBER_DATED) * INTERVAL_DAYS;
  const d = new Date(START_DATE);
  d.setDate(d.getDate() + offset);
  return toISO(d);
}

// Migration : on ne garde QUE les vidéos numérotées >= 31, et les slots vides du nouveau planning.
// - "Vidéo N" avec N < 31  → supprimée
// - "Vidéo N" avec N >= 31 → date assignée selon le nouveau rythme
// - Vidéo sans numéro       → supprimée
// - Slot vide               → remplacé par le nouveau planning
// En cas de conflit (deux "Vidéo 32"), on garde la plus complète.
function migrateAndMergeVideos(loaded) {
  const datedOverrides = {}; // { dateISO: video }

  for (const v of loaded) {
    const num = extractVideoNumber(v.title);
    if (num === null || num < 31) continue; // exclus

    const newDate = dateForVideoNumber(num);
    const id = `vid_${newDate}`;
    const candidate = { ...v, date: newDate, id };

    if (datedOverrides[newDate]) {
      // Conflit : on garde la plus complète
      if (videoCompleteness(candidate) > videoCompleteness(datedOverrides[newDate])) {
        datedOverrides[newDate] = candidate;
      }
    } else {
      datedOverrides[newDate] = candidate;
    }
  ...
  // Génère les slots vides du nouveau planning, en y injectant les overrides
  const fresh = generateSlots(START_DATE, DEFAULT_END);
  const merged = fresh.map(slot => datedOverrides[slot.date] || slot);

  // Vidéos numérotées dont la date dépasse DEFAULT_END (on les conserve quand même)
  const mergedDates = new Set(merged.map(v => v.date));
  const beyondEnd = Object.values(datedOverrides).filter(v => !mergedDates.has(v.date));

  // Garantit l'unicité des IDs (sécurité supplémentaire contre les bugs React)
  const seen = new Set();
  return [...merged, ...beyondEnd]
...
export default function BinovaPlanner() {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        if (v?.value) {
          // Chargement direct sans migration : les vidéos sont libres d'avoir n'importe quel titre.
          // On applique juste un dédoublonnage sur les IDs par sécurité (protège du bug React).
          const loaded = JSON.parse(v.value);
          const seen = new Set();
          const deduped = loaded.filter(item => {
  ...
  const deleteVideo = useCallback((id) => {
    ...
    (async () => { try { await window.storage.delete(thumbKey(id)); } catch {} })();
  }, []);

  const addVideo = useCallback(() => {
    // Date par défaut = aujourd'hui, ou 8 juin 2026 si on est encore avant
    const today = new Date();
    const cutoff = new Date(2026, 5, 8);
    const d = today < cutoff ? cutoff : today;
    d.setHours(0, 0, 0, 0);
    const dateISO = toISO(d);
    // ID unique — pas basé sur la date pour éviter les collisions
  ...
  const updateShort = useCallback((monthKey, patch) => {
    setShorts(prev => {
      const current = prev[monthKey] || { rushLink: '', status: 'rien', finalLink: '', posted: false };
      return { ...prev, [monthKey]: { ...current, ...patch } };
    });
  }, []);
  ...
  return (
    <div style={{
      ...
      <main style={{ maxWidth:1400, margin:'0 auto', padding:'48px 40px 80px' }}>
        {view === 'planning' && <PlanningView videos={videos} updateVideo={updateVideo} thumbs={thumbs} setVideoThumbnail={setVideoThumbnail} deleteVideo={deleteVideo} addVideo={addVideo} />}
...
//  PLANNING
// ============================================================
function PlanningView({ videos, updateVideo, thumbs, setVideoThumbnail, deleteVideo, addVideo }) {
  ...
  const filtered = useMemo(() => {
    ...
    // Tri par date pour que les vidéos apparaissent dans le bon ordre chronologique,
    // notamment quand l'utilisateur modifie manuellement une date.
  ...
  return (
    <div>
      ...
      <p style={{ fontSize:13, color:C.muted, margin:'8px 0 36px' }}>
        <span className="mono">{videos.length}</span> créneaux · 1 vidéo tous les 3 jours à partir du 8 juin 2026
      </p>
      ...
      <div style={{ display:'flex', gap:8, marginBottom:20, alignItems:'center' }}>
        ...
        <button
          ...
          style={{
            padding:'9px 14px', background:C.yellow, color:'#000', border:'none',
            borderRadius:7, fontSize:12, fontWeight:600, cursor:'pointer',
            display:'flex', alignItems:'center', gap:6, fontFamily:'inherit',
          }}
          title="Ajouter une nouvelle vidéo au planning"
        >
          <span style={{ fontSize:16, lineHeight:1 }}>+</span> Nouvelle vidéo
        </button>
      </div>
...
function Row({ video, expanded, onToggle, onUpdate, thumb, setThumbnail, onDelete }) {
  ...
  return (
    <div className="row" style={{ borderBottom:`1px solid ${C.line}` }}>
      <div style={{
        ...
        <DateCell value={video.date} onChange={date => onUpdate({ date })} />
      ...
      {expanded && (
        <div className="fade-in" style={{ padding:'4px 14px 20px', background:'#0b0b0e', borderTop:`1px solid ${C.line}` }}>
          ...
          <div style={{ display:'flex', justifyContent:'flex-end', marginTop:16, borderTop:`1px solid ${C.line}`, paddingTop:14 }}>
            <button
              onClick={handleDelete}
              style={{
                padding:'6px 12px',
                background: confirmDelete ? '#450a0a' : 'transparent',
                border:`1px solid ${confirmDelete ? '#7f1d1d' : C.line}`,
                borderRadius:6,
                color: confirmDelete ? '#fca5a5' : C.muted,
                fontSize:12, cursor:'pointer', fontFamily:'inherit',
                display:'flex', alignItems:'center', gap:6,
                transition:'all 0.12s',
              }}
              onMouseEnter={e => { if (!confirmDelete) e.currentTarget.style.borderColor = '#7f1d1d33'; }}
              onMouseLeave={e => { if (!confirmDelete) e.currentTarget.style.borderColor = C.line; }}
            >
              <Trash2 size={12} /> {confirmDelete ? 'Confirmer la suppression' : 'Supprimer cette vidéo'}
            </button>
          </div>
        </div>
      )}
...
// Cellule de date modifiable : affiche "Lun 08 jui" et ouvre le date picker natif au clic.
function DateCell({ value, onChange }) {
  ...
  const openPicker = () => {
    ...
    try {
      ...
      // Certains navigateurs restreignent showPicker() → fallback focus
      input.focus();
    }
  };
  ...
  return (
    ...
    >
      ...
      <input
        ...
        // input hors flux visuel mais fonctionnel : le picker s'ancre à sa position
        style={{
          position:'absolute',
          left:0, top:'100%',
          width:1, height:1,
...
const SHORTS_END_YEAR = 2027;
const SHORTS_END_MONTH = 11; // Décembre 2027 par défaut
...
function generateMonthBatches() {
  const list = [];
  let y = SHORTS_START_YEAR, m = SHORTS_START_MONTH;
  while (y < SHORTS_END_YEAR || (y === SHORTS_END_YEAR && m <= SHORTS_END_MONTH)) {
    const key = `${y}-${String(m + 1).padStart(2, '0')}`;
    list.push({ key, year: y, month: m });
    m++;
    if (m > 11) { m = 0; y++; }
  }
  return list;
...
function ShortsView({ shorts, updateShort }) {
  const batches = useMemo(() => generateMonthBatches(), []);
  const stats = useMemo(() => {
    let posted = 0, total = 0;
    for (const b of batches) {
      total++;
      const data = shorts[b.key];
  ...
  return (
    <div>
      ...
      <p style={{ fontSize:13, color:C.muted, margin:'8px 0 36px' }}>
        Batch mensuel · 30 shorts par mois · <span className="mono">{stats.posted}/{stats.total}</span> postés
      </p>

      <div style={{ borderTop:`1px solid ${C.line}`, borderRadius:'10px 10px 0 0', overflow:'hidden' }}>
        <div style={{
          display:'grid', gridTemplateColumns:'200px 150px 1fr 1fr 90px',
          padding:'10px 14px', borderBottom:`1px solid ${C.line}`,
          fontSize:10, letterSpacing:'0.1em', textTransform:'uppercase',
          color:C.muted2, gap:14, background:C.bg2,
        }}>
          <div>Batch</div>
          <div>Statut</div>
          <div>Rushes</div>
          <div>Shorts terminés</div>
          <div style={{ textAlign:'center' }}>Posté</div>
        </div>
        <div className="scroll" style={{ maxHeight:'70vh', overflowY:'auto' }}>
          {batches.map(b => (
            <ShortBatchRow
              key={b.key}
              batch={b}
              data={shorts[b.key] || { rushLink:'', status:'rien', finalLink:'', posted:false }}
              onUpdate={patch => updateShort(b.key, patch)}
            />
          ))}
        </div>
      </div>
...
function ShortBatchRow({ batch, data, onUpdate }) {
  const status = SHORT_STATUSES[data.status] || SHORT_STATUSES.rien;
  return (
    <div className="row" style={{
      ...
      <div>
        <div style={{ fontSize:14, fontWeight:600, color:C.text }}>
          {MONTHS[batch.month]} <span className="mono" style={{ color:C.muted, fontWeight:400 }}>{batch.year}</span>
        </div>
        <div style={{ fontSize:11, color:C.muted2, marginTop:2 }}>30 shorts</div>
      </div>

      <div style={{ display:'flex', alignItems:'center', gap:8, minWidth:0 }}>
        <Dot color={status.color} withGlow={data.status !== 'rien'} />
        <select
          value={data.status}
          onChange={e => onUpdate({ status: e.target.value })}
          style={{
            background:'transparent', border:'none',
            color: data.status === 'rien' ? C.muted : C.text,
            fontSize:13, cursor:'pointer', padding:0, flex:1, minWidth:0,
          }}
        >
          {Object.entries(SHORT_STATUSES).map(([k, st]) => (
            <option key={k} value={k} style={{ background:C.bg, color:C.text }}>{st.label}</option>
          ))}
        </select>
      </div>

      <LinkCell value={data.rushLink} onChange={v => onUpdate({ rushLink: v })} />
      <LinkCell value={data.finalLink} onChange={v => onUpdate({ finalLink: v })} />

      <div style={{ display:'flex', justifyContent:'center' }}>
        <TelegramCheck
          checked={!!data.posted}
          onChange={val => onUpdate({ posted: val })}
        />
      </div>
    </div>
  );
...
function Settings({ settings, setSettings, onClose, videos, setVideos }) {
  ...
  const regenerate = () => {
    ...
    setVideos(mergePlanningPreserving(videos, START_DATE, DEFAULT_END));
    setConfirmReset(false);
  };

  const extendTo = year => {
    setVideos(mergePlanningPreserving(videos, START_DATE, new Date(year, 11, 31)));
  };
